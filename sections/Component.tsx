...
export const useComponent = <T = Record<string, unknown>>(component: string, props?: T, otherProps: {
    href?: string;
} = {}) => useSection({
    ...otherProps,
    props: {
...
export const loader = async ({ component, props }: Props, req: Request, ctx: AppContext) => {
    const { default: Component, loader, action } = await import(`${ROOT}${component}`);
    return {
...
export const action = async ({ component, props }: Props, req: Request, ctx: AppContext) => {
    const { default: Component, action, loader } = await import(`${ROOT}${component}`);
    return {
